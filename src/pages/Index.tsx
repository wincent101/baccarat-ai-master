...
import { BaccaratResult, PredictionResult, predict, computeStats } from "@/lib/baccaratEngine";
...
export default function Index() {
  ...
  const handleResult = useCallback((result: BaccaratResult) => {
    setHistory((prev) => {
      const nextPrediction = predict(prev);
      setPredictions((current) => [...current, nextPrediction.result]);
      return [...prev, result];
    });
  }, []);
